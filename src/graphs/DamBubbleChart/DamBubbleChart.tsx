...
const DamBubbleChart: React.FC<Props> = ({ data, width = 1360, height = 1020 }) => {
  ...
  const nodes = useMemo(() => {
    ...
    const pack = d3.pack<any>().size([width, chartHeight]).padding(6);
  ...
  // Estimate if text will fit in the circle
  const textFitsInCircle = (damName: string, radius: number) => {
    // Approximate character width in pixels (rough estimate for the font size used)
    const charWidth = 7;
